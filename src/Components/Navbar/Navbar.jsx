    ...
    >
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <button
          ...
          type="button"
          className="inline-flex items-center p-2 py-4 ml-3 text-sm text-slate-200 md:hidden"
        ...
        <div className={`${menuOpen ? 'block' : 'hidden'} w-full md:block md:w-auto`}>
          <ul
            className={`font-medium flex flex-col p-4 md:p-0 mt-4 rounded-lg md:flex-row md:space-x-8 md:mt-0
              ${menuOpen ? 'bg-gray-900 text-white' : 'md:bg-transparent'}`
            }
          >
            {['home', 'about', 'services', 'events', 'contact'].map((section) => (
              <li key={section} className={`${menuOpen ? 'border-b border-gray-700 last:border-0' : ''}`}>
                <Link
                  to={section}
                  smooth={true}
                  duration={800}
